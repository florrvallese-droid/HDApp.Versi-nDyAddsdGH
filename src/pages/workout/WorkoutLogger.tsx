...
import { Trash2, Plus, Clock, ChevronLeft, Target, Loader2 } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { supabase } from "@/services/supabase";
import { toast } from "sonner";
import { WorkoutExercise, WorkoutSet, WorkoutData } from "@/types";
...
const findBestSet = (sets: WorkoutSet[]): { weight: number; reps: number } => {
  if (!sets || sets.length === 0) return { weight: 0, reps: 0 };
  ...
  const startWorkout = async () => {
    ...
    try {
      ...
      if (lastLog && lastLog.data) {
        const prevData = lastLog.data as WorkoutData;
        toast.info("Cargando tu última sesión para superar.");

        const prefilledExercises = prevData.exercises.map((ex: WorkoutExercise) => ({
          name: ex.name,
          sets: [{ weight: 0, reps: 0, rpe: 8 }],
          previous: findBestSet(ex.sets)
        }));
        setExercises(prefilledExercises);
      } else {
        toast.info("No hay sesión anterior. ¡A darle con todo!");
        setExercises([]);
      }
      ...
      toast.error("Error al cargar sesión anterior: " + error.message);
    } finally {
      setLoading(false);
    }
  };
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <Button className="w-full" onClick={startWorkout} disabled={!muscleGroup || loading}>
              {loading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Cargando...</> : "Comenzar"}
            </Button>
          </CardContent>
        </Card>
      </div>
        ...
        {exercises.map((exercise, exIndex) => (
          <Card key={exIndex} className="relative overflow-hidden">
            ...
            <CardContent className="p-3 space-y-3">
              {exercise.previous && (exercise.previous.weight > 0 || exercise.previous.reps > 0) && (
                <div className="bg-blue-500/10 border-l-4 border-blue-500 text-blue-700 dark:text-blue-300 p-3 rounded-md mb-3">
                  <p className="text-xs font-bold uppercase tracking-wider flex items-center gap-2">
                    <Target className="h-3 w-3" />
                    Objetivo a Superar
                  </p>
                  <p className="font-mono text-lg font-bold">{exercise.previous.weight}{profile?.units || 'kg'} x {exercise.previous.reps} reps</p>
                </div>
              )}
